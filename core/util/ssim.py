...
def ssim_per_patient(full_patient_list,version, surgery,db):
    import shutil
    ...
    # full_patient_list.remove("/workspace/raid/NRS/lapa/vihub/img/01_VIHUB1.2_A9_L_2")
    ...
    for patient in full_patient_list: 
        ...
        if db == "yes":
            for video in video_path:
                ...
                json_name = video.split("/")[-1]
                ori_anno = video+"/anno/v1/"+ json_name +".json"
                print('\n+[origin anno]  : {}'.format(ori_anno))

                target_anno_path = video+"/anno/v3/"
                if os.path.exists(target_anno_path):
                    target_anno = target_anno_path + json_name+".json"
            ...
            for video in video_path:
                # NRS 이미지만 ssim 계산
                json_name = video.split("/")[-1]
                anno_list = os.listdir( args.data_base_path+"/" + surgery + "/vihub/anno/v1/")
                for j in range(len(anno_list)):
                    if anno_list[j].startswith(json_name):
                        ori_anno = args.data_base_path+"/" + surgery + "/vihub/anno/v1/"+  anno_list[j] 
                        print('\n+[origin anno]  : {}'.format(ori_anno))

                        target_anno_path = args.data_base_path+"/" + surgery +"/vihub/anno/v3/"
                        if os.path.exists(target_anno_path):
                            target_anno = target_anno_path + json_name+".json"
                            shutil.copy(ori_anno,target_anno)
                        else:
                        ...
                        print('+[target frames] : {}'.format(video))
                        target_frames_list = os.listdir(video)
                        target_frames=[]
                        for j in range(len(target_frames_list)):
                            target_frames.append(video + "/"+ target_frames_list[j])
                        target_frames = natsort.natsorted(target_frames)
                        ...
                        # ######### calculate ssim score #########
                        final_df_1_fps = compute_ssim(version,target_assets_df=patient_df, ssim_score_th=0.997, n_cpu=10)
                        print('\n\n\t\t<< final_df_1_fps >>\n')
                        print(final_df_1_fps) 
...
def compute_ssim(version,target_assets_df, ssim_score_th, n_cpu):
    import numpy as np
    import pandas as pd
    ...
    if version == "cpu":
        ...
        ssim_score_list = calculate_ssim_score(target_ssim_list, 0, len(target_ssim_list))


    # TODO
    print( len(target_ssim_df['frame_path']))
    print( len(ssim_score_list))
    if len(target_ssim_df['frame_path']) != len(ssim_score_list):
        min_len = min(len(img_list), len(label_list))
        ssim_score_list = ssim_score_list[:min_len]
        target_ssim_df = target_ssim_df[:min_len]
