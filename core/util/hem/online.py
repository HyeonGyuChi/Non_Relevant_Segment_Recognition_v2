...
class OnlineHEM():
    ...
    def apply(args, emb, y_hat, proxies):
        # upate 방법 별 분기
        ...
        if sum(wrong_answer) > 0:
            ...
            classification_loss3, proxy_loss3 = self.compute_hard(proxies, neg_sim_mat, neg_y_hat, neg_y, neg_emb)
    ...
    def compute_hard(self, proxies, neg_sim_mat, neg_y_hat, neg_y, neg_emb):  
        proxy_loss = 0
        classification_loss = 0        
        ...
        if self.args.use_step_weight:
            ...
            w = torch.Tensor(np.array([torch.exp(self.alpha - wrong_sim_dist[wi]) for wi in range(len(wrong_sim_dist))])).cuda()
            
        if neg_emb is not None:
            b = torch.nn.functional.normalize(proxies[:, 0:1].transpose(0,1))
            c = torch.nn.functional.normalize(proxies[:, 1:2].transpose(0,1))
            
        for wi in range(len(neg_sim_mat)):
            if neg_emb is not None:            
                a = neg_emb[wi:wi+1, ]                
